...
/*
    Fast register-level unpacking of 8 INT4 values from a 32-bit register into 4
   half2 registers.
 */
__device__ __forceinline__ void
unpack_int4_to_fp16_v8(uint32_t val, half *out_fp16, half scale, half zero) {
#pragma unroll
  for (int i = 0; i < 8; ++i) {
 ...
 */
__global__ void int4_gemm_dequant_kernel(const half *__restrict__ A,
  ...
  // Temp register buffer for unpacked FP16 weights (16 elements per WMMA tile
  // step)
  ...
  for (int k_idx = 0; k_idx < K; k_idx += WMMA_K) {
    ...
    for (int row = 0; row < WMMA_K; ++row) {
      ...
      for (int col = lane_id; col < WMMA_N; col += 32) {
        ...
        half zero = (zeros != nullptr) ? zeros[group_idx * N + col_idx]
                                       : __float2half(0.0f);

        float fval = static_cast<float>(nibble);
        unpacked_w[row * WMMA_N + col] =
            __hmul(__hsub(__float2half(fval), zero), scale);
      }
    }
    __syncwarp();
  ...
  wmma::store_matrix_sync(C + m_offset * N + n_offset, c_frag, N,
                          wmma::mem_row_major);
}

void launch_int4_gemm_dequant(const Int4GemmParams &params,
                              cudaStream_t stream) {
  if (params.K % 16 != 0 || params.N % 16 != 0 || params.M % 16 != 0) {
    throw std::invalid_argument(
        "INT4 GEMM dimensions (M, N, K) must be multiples of 16.");
  }

  dim3 threads_per_block(4, 4); // 16 warps = 512 threads per block
  dim3 num_blocks((params.N + (WMMA_N * threads_per_block.x) - 1) /
                      (WMMA_N * threads_per_block.x),
                  (params.M + (WMMA_M * threads_per_block.y) - 1) /
                      (WMMA_M * threads_per_block.y));

  int4_gemm_dequant_kernel<<<num_blocks, threads_per_block, 0, stream>>>(
      params.A, params.B_quant, params.scales, params.zeros, params.C, params.M,
