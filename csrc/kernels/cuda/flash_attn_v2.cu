...
/**
 * @brief Inline PTX instruction for Ampere cp.async (Global -> Shared Memory
 * direct DMA)
 */
...
/*
    Ampere FlashAttention-2 CUDA Kernel using Online Softmax and Shared Memory
   Tiling.
 */
...
__global__ void flash_attn_v2_ampere_kernel(
    ...
    const int num_heads, const float sm_scale, const bool is_causal) {
  ...
  const size_t bh_offset =
      (static_cast<size_t>(bz) * num_heads + by) * seq_len * HEAD_DIM;
  ...
  for (int i = 0; i < q_bytes_per_thread; ++i) {
    ...
    if (q_start + row < seq_len) {
      cp_async_cg_16(&smem_q[row * HEAD_DIM + col],
                     &q_ptr[row * HEAD_DIM + col]);
    }
  }
  cp_async_commit();
  ...
  // Loop over K/V tiles
  const int max_k_tiles = is_causal
                              ? min((q_start + BLOCK_M + BLOCK_N - 1) / BLOCK_N,
  ...
  for (int k_tile = 0; k_tile < max_k_tiles; ++k_tile) {
    ...
    for (int i = 0; i < q_bytes_per_thread; ++i) {
      ...
      if (k_start + row < seq_len) {
        cp_async_cg_16(&smem_k[row * HEAD_DIM + col],
                       &k_ptr[(k_start + row) * HEAD_DIM + col]);
        cp_async_cg_16(&smem_v[row * HEAD_DIM + col],
                       &v_ptr[(k_start + row) * HEAD_DIM + col]);
      }
    }
    cp_async_commit();
    cp_async_wait_group<0>();
    ...
    if (q_start + q_row_local < seq_len) {
      for (int n = 0; n < BLOCK_N; ++n) {
        if (k_start + n >= seq_len)
          continue;
        if (is_causal && (k_start + n > q_start + q_row_local))
          continue;
    ...
    // 5. Update Output Accumulator: O = O * (l_prev * alpha / l_curr) + (P * V)
    // / l_curr
    ...
    if (q_start + q_row_local < seq_len) {
      for (int n = 0; n < BLOCK_N; ++n) {
        if (s_scores[n] > 0.0f) {
          ...
          for (int d = 0; d < HEAD_DIM / 4; ++d) {
            float v_val =
                __half2float(smem_v[n * HEAD_DIM + (d * 4 + (tid % 4))]);
            o_acc[d] += p_val * v_val;
          }
        }
      }
...
void launch_flash_attn_v2(const FlashAttnParams &params, cudaStream_t stream) {
  if (params.head_dim != 64 && params.head_dim != 128) {
    throw std::invalid_argument(
        "FlashAttention-2 Kernel only supports head_dim 64 or 128");
  }

  dim3 grid((params.seq_len + BLOCK_M - 1) / BLOCK_M, params.num_heads,
            params.batch_size);
  ...
  if (params.head_dim == 64) {
    flash_attn_v2_ampere_kernel<64><<<grid, block, smem_bytes, stream>>>(
        params.Q, params.K, params.V, params.O, params.seq_len,
        params.num_heads, params.sm_scale, params.is_causal);
