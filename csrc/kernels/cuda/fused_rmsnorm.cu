...
/*
    Fused Residual Addition + RMSNorm CUDA Kernel with 128-bit Vectorized Memory
   Access.
 */
__global__ void fused_rmsnorm_kernel(const half *__restrict__ input,
  ...
  // Phase 1: Read input & residual, accumulate sum of squares, write back
  // updated residual
...
void launch_fused_rmsnorm(const FusedRMSNormParams &params,
                          cudaStream_t stream) {
  if (params.hidden_dim % 8 != 0) {
    throw std::invalid_argument("fused_rmsnorm requires hidden_dim to be a "
                                "multiple of 8 for 128-bit vectorization.");
  }

  int block_size = 256;
  if (params.hidden_dim / 8 < block_size) {
    block_size = std::max(32, (params.hidden_dim / 8 + WARP_SIZE - 1) /
                                  WARP_SIZE * WARP_SIZE);
  }
  ...
  fused_rmsnorm_kernel<<<grid, block, shared_mem_bytes, stream>>>(
      params.input, params.residual, params.weight, params.output,
      params.hidden_dim, params.epsilon);
}
