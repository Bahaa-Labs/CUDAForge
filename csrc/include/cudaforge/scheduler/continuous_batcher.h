...
enum class RequestState { WAITING, RUNNING, FINISHED, PREEMPTED };
...
struct Request {
  ...
  Request(uint64_t req_id, std::vector<int32_t> prompt, int32_t max_tokens,
          int32_t prio = 0)
      : id(req_id), prompt_tokens(std::move(prompt)),
        max_new_tokens(max_tokens), priority(prio),
  ...
  bool is_finished() const {
    return state == RequestState::FINISHED ||
           generated_tokens.size() >= static_cast<size_t>(max_new_tokens);
  }
};

/*
    Priority Comparator: Highest priority first, then First-Come-First-Served
   (FCFS).
 */
...
public:
  ...
  /*
      Schedules requests for the current iteration step under token budget
     constraints.
   */
