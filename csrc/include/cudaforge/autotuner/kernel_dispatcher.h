...
/*
    Unique Key structure representing problem dimensions and type
   configurations.
 */
struct KernelKey {
  std::string op_name;
  ...
  bool operator==(const KernelKey &other) const {
    return op_name == other.op_name && M == other.M && N == other.N &&
           K == other.K && dtype_code == other.dtype_code;
  }
};
...
public:
  ...
  /**
   * @brief Dispatches the best cached kernel or benchmarks candidate functions
   * to elect the fastest variant.
   *
  ...
  std::string autotune_candidates(
    ...
    if (candidates.empty()) {
      throw std::runtime_error(
          "Autotuner error: No candidate kernels provided.");
    }
