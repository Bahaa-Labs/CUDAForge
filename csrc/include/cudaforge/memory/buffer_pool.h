...
/*
    Exception raised when buffer allocation fails even after multi-stage OOM purging.
 */
...
/*
    Stream-aware, bucketed GPU buffer pool with fragmentation metrics & OOM recovery.
 */
class BufferPool {
public:
    /**
     ...
     * @param max_cached_bytes Max VRAM bytes allowed to stay cached in free buckets (default 2GB).
     */
    explicit BufferPool(BlockAllocator& allocator, size_t max_cached_bytes = 2ULL * 1024 * 1024 * 1024);
    ~BufferPool();
    ...
    /**
     * @brief Acquires a reusable buffer from pool or allocates new VRAM via BlockAllocator.
     * Executes emergency purging if BlockAllocator throws an OOM exception.
     ...
     */
    [[nodiscard]] void* acquire(size_t bytes, MemoryCategory category = MemoryCategory::WORKSPACE, cudaStream_t stream = nullptr);
    ...
    /**
     * @brief Flushes unused cached free buffers back to BlockAllocator to free physical VRAM.
    ...
    /**
     * @brief Creates an RAII ScopedBuffer container that auto-releases upon going out of scope.
     */
    [[nodiscard]] ScopedBuffer acquire_scoped(size_t bytes, MemoryCategory category = MemoryCategory::WORKSPACE, cudaStream_t stream = nullptr);
...
private:
    ...
    [[nodiscard]] bool attempt_allocation(size_t bucket_bytes, size_t requested_bytes, MemoryCategory category, cudaStream_t stream, void** out_ptr);
...
public:
    ...
    ScopedBuffer(BufferPool* pool, void* ptr, size_t bytes, cudaStream_t stream = nullptr)
    ...
    ScopedBuffer(ScopedBuffer&& other) noexcept
        : pool_(other.pool_), ptr_(other.ptr_), bytes_(other.bytes_), stream_(other.stream_) {
        other.pool_ = nullptr;
        other.ptr_ = nullptr;
        other.bytes_ = 0;
    ...
    template <typename T>
    [[nodiscard]] T* get_typed() const noexcept { return static_cast<T*>(ptr_); }
