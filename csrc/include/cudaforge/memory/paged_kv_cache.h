...
#define CUDA_CHECK_METADATA(call)                                              \
  do {                                                                         \
    ...
    if (err != cudaSuccess) {                                                  \
      throw std::runtime_error(std::string("CUDA Error in PagedKVCache: ") +   \
...
/*
    Plain C-compatible Batch Descriptor struct passed directly to CUDA device
   kernels.
 */
struct KVCacheBatchDescriptor {
  const int32_t *block_tables{
      nullptr}; // GPU pointer: [batch_size, max_blocks_per_seq]
...
public:
  ...
  uint32_t
  register_sequence_with_prefix(uint64_t sequence_id,
  ...
  /**
   * @brief Asynchronously prepares and stages GPU metadata structures for a
   * batch of sequences.
   ...
   * @return KVCacheBatchDescriptor with valid device pointers ready for kernel
   * launch.
   */
  KVCacheBatchDescriptor
  prepare_batch_metadata(const std::vector<uint64_t> &active_sequence_ids,
                         cudaStream_t stream = 0);

  [[nodiscard]] const std::vector<int32_t> &
  get_block_table(uint64_t sequence_id) const;
  [[nodiscard]] void *get_key_block_ptr(int32_t block_id,
                                        uint32_t layer_idx) const;
  [[nodiscard]] void *get_value_block_ptr(int32_t block_id,
                                          uint32_t layer_idx) const;

  [[nodiscard]] const KVCacheConfig &get_config() const noexcept {
    return config_;
  }
...
private:
  ...
  compute_block_hash(std::span<const int32_t> tokens,
                     uint64_t parent_hash = 0) noexcept;
  ...
  std::unordered_map<uint64_t, std::shared_ptr<PrefixTreeNode>>
      hash_to_node_map_;
