 ...
 */
[[nodiscard]] constexpr std::string_view category_to_string(MemoryCategory category) noexcept {
    switch (category) {
        case MemoryCategory::WEIGHTS:     return "WEIGHTS";
        case MemoryCategory::ACTIVATIONS: return "ACTIVATIONS";
        case MemoryCategory::WORKSPACE:   return "WORKSPACE";
        case MemoryCategory::KV_CACHE:    return "KV_CACHE";
        default:                          return "UNKNOWN";
    }
}

/*
    Exception thrown when an allocation request exceeds the configured VRAM safety budget.
 */
...
struct MemoryStats {
    ...
    std::array<size_t, static_cast<size_t>(MemoryCategory::NUM_CATEGORIES)> category_bytes{};
};
...
public:
     ...
     * @param vram_headroom_fraction Percentage of VRAM reserved for CUDA driver overhead (default 5% -> 95% ceiling).
     */
     ...
     * @param stream CUDA stream associated with execution context (default nullptr / default stream).
     * @return Raw GPU void pointer to memory block.
     * @throws OutOfMemoryException if budget or physical VRAM is exceeded.
     */
    [[nodiscard]] void* allocate(size_t bytes, MemoryCategory category, cudaStream_t stream = nullptr);

    /**
     * @brief Strongly typed allocator wrapper for element arrays.
     */
    template <typename T>
    [[nodiscard]] T* allocate_typed(size_t count, MemoryCategory category, cudaStream_t stream = nullptr) {
    ...
    /**
     * @brief Queries physical VRAM from CUDA driver and updates internal stats snapshot.
     */
...
private:
    [[nodiscard]] static size_t align_bytes(size_t bytes, size_t alignment = 256) noexcept;
    ...
    std::array<size_t, static_cast<size_t>(MemoryCategory::NUM_CATEGORIES)> category_allocated_bytes_{};
};
