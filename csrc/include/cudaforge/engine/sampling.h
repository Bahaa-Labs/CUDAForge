 ...
 */
__global__ void fused_sampling_kernel(const half *__restrict__ logits,
 ...
 */
inline void launch_fused_sampling(const SamplingParams &params,
                                  cudaStream_t stream = 0) {
  if (params.batch_size <= 0 || params.vocab_size <= 0) {
    throw std::invalid_argument(
        "Invalid batch size or vocab size for sampling launch.");
  }
  ...
  detail::fused_sampling_kernel<<<grid, block, shared_mem_bytes, stream>>>(
      params.logits, params.output_tokens, params.vocab_size,
      params.temperature, params.top_k, params.top_p, params.rng_seed);
#else
  throw std::runtime_error(
      "launch_fused_sampling must be compiled with NVCC (__CUDACC__).");
#endif
}
