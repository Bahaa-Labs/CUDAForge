...
/*
    Applies Rotary Position Embedding (RoPE) to a single Q/K vector pair in
   register/shared memory.
 */
template <int HEAD_DIM>
__device__ __forceinline__ void
apply_rope_vector(half *__restrict__ vec, const float *__restrict__ cos_ptr,
                  const float *__restrict__ sin_ptr, const int tid) {
...
__global__ void rope_kernel(
    ...
    const int num_q_heads, const int num_kv_heads, const int seq_len) {
  const int token_idx =
      blockIdx.x; // Token index in combined batch*seq dimension
  ...
  if (is_key == 0) {
    if (head_idx >= num_q_heads)
      return;
    size_t offset =
        (static_cast<size_t>(token_idx) * num_q_heads + head_idx) * HEAD_DIM;
    apply_rope_vector<HEAD_DIM>(Q + offset, cos_row, sin_row, tid);
  } else {
    if (head_idx >= num_kv_heads)
      return;
    size_t offset =
        (static_cast<size_t>(token_idx) * num_kv_heads + head_idx) * HEAD_DIM;
