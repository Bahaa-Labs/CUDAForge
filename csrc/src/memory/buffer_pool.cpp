...
BufferPool::BufferPool(BlockAllocator &allocator, size_t max_cached_bytes)
  ...
  spdlog::info("BufferPool initialized | Max Cached Capacity Limit: {:.2f} MB",
...
BufferPool::~BufferPool() {
  ...
  if (active_bytes_ > 0) {
    spdlog::warn("BufferPool destroyed with {:.2f} MB active allocations still "
                 "unreleased!",
...
bool BufferPool::attempt_allocation(size_t bucket_bytes, size_t requested_bytes,
  ...
  try {
    ...
    *out_ptr = d_ptr;
    ...
    PooledBufferNode node{.ptr = d_ptr,
...
void *BufferPool::acquire(size_t bytes, MemoryCategory category,
                          cudaStream_t stream) {
  if (bytes == 0) {
    return nullptr;
  }
  ...
  if (it != free_buckets_.end() && !it->second.empty()) {
    ...
    spdlog::trace("BufferPool Hit | Address: {:p} | Bucket: {} KB", d_ptr,
                  bucket_size / 1024);
    return d_ptr;
  }
  ...
  if (attempt_allocation(bucket_size, bytes, category, stream, &d_ptr)) {
    spdlog::trace("BufferPool Miss (Allocated) | Address: {:p} | Bucket: {} KB",
                  d_ptr, bucket_size / 1024);
    return d_ptr;
  }

  // 3. Low-Level OOM Recovery Path
  spdlog::warn("BufferPool OOM Encountered. Initiating Recovery Purge for "
               "request size {:.2f} MB...",
  ...
  if (attempt_allocation(bucket_size, bytes, category, stream, &d_ptr)) {
    spdlog::info("BufferPool Recovery Successful! Purged {:.2f} MB VRAM to "
                 "satisfy {:.2f} MB request.",
  ...
  std::string err_msg =
      fmt::format("BufferPool OOM Failure: Exceeded GPU memory limits even "
                  "after recovery purge!\n"
...
void BufferPool::release(void *ptr, cudaStream_t stream) {
  ...
  if (node_it == pool_nodes_.end()) {
    spdlog::error("BufferPool::release attempted on untracked pointer: {:p}",
                  ptr);
    return;
  }
  ...
  if (!node.in_use) {
    spdlog::warn(
        "BufferPool::release called on pointer {:p} which is already free!",
        ptr);
    return;
  }
  ...
  // If cached VRAM exceeds max_cached_bytes_ threshold, trim oldest cached
  // buffers
  if (free_cached_bytes_ > max_cached_bytes_) {
    spdlog::debug(
        "BufferPool cache limit exceeded ({:.2f} MB > {:.2f} MB). Trimming...",
...
size_t BufferPool::purge_free_buffers(cudaStream_t stream) {
  ...
  spdlog::info("BufferPool Purged {} bytes back to BlockAllocator.",
               bytes_purged);
  return bytes_purged;
}

ScopedBuffer BufferPool::acquire_scoped(size_t bytes, MemoryCategory category,
                                        cudaStream_t stream) {
...
BufferPoolStats BufferPool::get_stats() const {
  ...
  if (active_bytes_ > 0) {
    stats.internal_fragmentation_ratio =
        1.0 - (static_cast<double>(total_requested_bytes_) /
               static_cast<double>(active_bytes_));
  } else {
  ...
  if (free_cached_bytes_ > 0) {
    ...
    stats.external_fragmentation_ratio =
        1.0 - (static_cast<double>(largest_free_block) /
               static_cast<double>(free_cached_bytes_));
  } else {
...
void BufferPool::dump_pool_state() const {
  ...
  spdlog::info(" Total Pooled Memory:    {:.2f} MB",
               static_cast<double>(stats.total_pooled_bytes) /
                   (1024.0 * 1024.0));
  spdlog::info(" Active In-Use Memory:   {:.2f} MB",
               static_cast<double>(stats.active_bytes) / (1024.0 * 1024.0));
  spdlog::info(" Free Cached Memory:     {:.2f} MB",
               static_cast<double>(stats.free_cached_bytes) /
                   (1024.0 * 1024.0));
  spdlog::info(" Peak Historical Pool:   {:.2f} MB",
               static_cast<double>(stats.peak_pooled_bytes) /
                   (1024.0 * 1024.0));
  spdlog::info(" Internal Fragmentation: {:.2f}%",
               stats.internal_fragmentation_ratio * 100.0);
  spdlog::info(" External Fragmentation: {:.2f}%",
               stats.external_fragmentation_ratio * 100.0);
  spdlog::info(" Allocations Count:      {} (Hits: {}, Misses: {})",
               stats.total_allocations, stats.pool_hits, stats.pool_misses);
