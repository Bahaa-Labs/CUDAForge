...
BlockAllocator::BlockAllocator(double vram_headroom_fraction) {
  ...
  if (err != cudaSuccess) {
    spdlog::critical("BlockAllocator: Failed to query CUDA driver memory: {}",
                     cudaGetErrorString(err));
    throw std::runtime_error(
        "CUDA Driver query failed during BlockAllocator initialization.");
  }
  ...
  vram_safety_threshold_bytes_ = static_cast<size_t>(
      static_cast<double>(total_vram_bytes_) * usable_fraction);

  spdlog::info("BlockAllocator initialized | Total VRAM: {:.2f} GB | Safety "
               "Limit: {:.2f} GB (Headroom: {:.1f}%)",
               static_cast<double>(total_vram_bytes_) /
                   (1024.0 * 1024.0 * 1024.0),
               static_cast<double>(vram_safety_threshold_bytes_) /
                   (1024.0 * 1024.0 * 1024.0),
...
BlockAllocator::~BlockAllocator() {
  ...
  if (!active_allocations_.empty()) {
    spdlog::warn("BlockAllocator destructed with {} active unfreed allocations "
                 "totaling {:.2f} MB!",
...
void BlockAllocator::verify_cuda_error(cudaError_t err,
                                       std::string_view action_desc) const {
  if (err != cudaSuccess) {
    std::string err_msg =
        fmt::format("CUDA Error during '{}': {} (code {})", action_desc,
...
void *BlockAllocator::allocate(size_t bytes, MemoryCategory category,
                               cudaStream_t stream) {
  if (bytes == 0) {
    return nullptr;
  }
  ...
  if (projected_allocated > vram_safety_threshold_bytes_) {
    ...
    std::string oom_detail = fmt::format(
        ...
        static_cast<double>(category_allocated_bytes_[static_cast<size_t>(
  ...
  lock.unlock(); // Release lock while making driver call to prevent blocking
                 // other threads
  ...
  if (err != cudaSuccess) {
    std::string err_msg =
        fmt::format("cudaMallocAsync failed for {:.2f} MB in category '{}': {}",
                    static_cast<double>(aligned_bytes) / (1024.0 * 1024.0),
                    category_to_string(category), cudaGetErrorString(err));
    spdlog::error("{}", err_msg);
  ...
  peak_allocated_bytes_ =
      std::max(peak_allocated_bytes_, total_allocated_bytes_);

  spdlog::trace("Allocated block {:p} | Size: {:.2f} KB | Category: {}", d_ptr,
                static_cast<double>(aligned_bytes) / 1024.0,
                category_to_string(category));
...
void BlockAllocator::free(void *ptr, cudaStream_t stream) {
  ...
  if (it == active_allocations_.end()) {
    spdlog::error(
        "BlockAllocator::free attempted to deallocate untracked pointer: {:p}",
        ptr);
    return;
  }
  ...
  category_allocated_bytes_[static_cast<size_t>(handle.category)] -=
      handle.size_bytes;
  ...
  if (err != cudaSuccess) {
    spdlog::error("cudaFreeAsync failed for pointer {:p}: {}", ptr,
                  cudaGetErrorString(err));
  } else {
    spdlog::trace("Freed block {:p} | Released: {:.2f} KB", ptr,
                  static_cast<double>(handle.size_bytes) / 1024.0);
  }
}
...
void BlockAllocator::dump_memory_map() const {
  ...
  spdlog::info(" Physical VRAM Total:      {:.2f} GB",
               static_cast<double>(stats.total_vram_bytes) /
                   (1024.0 * 1024.0 * 1024.0));
  spdlog::info(" Physical VRAM Free:       {:.2f} GB",
               static_cast<double>(stats.free_vram_bytes) /
                   (1024.0 * 1024.0 * 1024.0));
  spdlog::info(" Allocator Limit Ceiling:  {:.2f} GB",
               static_cast<double>(stats.safety_limit_bytes) /
                   (1024.0 * 1024.0 * 1024.0));
  spdlog::info(" Active Allocated Total:   {:.2f} MB",
               static_cast<double>(stats.currently_allocated_bytes) /
                   (1024.0 * 1024.0));
  spdlog::info(" Peak Allocated Historical: {:.2f} MB",
  ...
  spdlog::info(
      "   - WEIGHTS:             {:.2f} MB",
      static_cast<double>(
          stats.category_bytes[static_cast<size_t>(MemoryCategory::WEIGHTS)]) /
          (1024.0 * 1024.0));
  spdlog::info("   - ACTIVATIONS:         {:.2f} MB",
               static_cast<double>(stats.category_bytes[static_cast<size_t>(
                   MemoryCategory::ACTIVATIONS)]) /
                   (1024.0 * 1024.0));
  spdlog::info("   - WORKSPACE:           {:.2f} MB",
               static_cast<double>(stats.category_bytes[static_cast<size_t>(
                   MemoryCategory::WORKSPACE)]) /
                   (1024.0 * 1024.0));
  spdlog::info(
      "   - KV_CACHE:            {:.2f} MB",
      static_cast<double>(
          stats.category_bytes[static_cast<size_t>(MemoryCategory::KV_CACHE)]) /
          (1024.0 * 1024.0));
}
