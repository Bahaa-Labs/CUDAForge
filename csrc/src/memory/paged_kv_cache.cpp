...
PagedKVCache::PagedKVCache(const KVCacheConfig& config, BlockAllocator& allocator)
    ...
    const size_t max_table_elements = config_.max_supported_batch_size * config_.max_blocks_per_sequence;
    ...
    CUDA_CHECK_METADATA(cudaHostAlloc(
        reinterpret_cast<void**>(&h_pinned_context_lens_), 
        config_.max_supported_batch_size * sizeof(int32_t), 
        cudaHostAllocDefault
    ));

    // Allocate Device Staging Memory for CUDA Kernel Consumption
    CUDA_CHECK_METADATA(cudaMalloc(
        reinterpret_cast<void**>(&d_block_tables_), 
        max_table_elements * sizeof(int32_t)
    ));
    CUDA_CHECK_METADATA(cudaMalloc(
        reinterpret_cast<void**>(&d_context_lens_), 
        config_.max_supported_batch_size * sizeof(int32_t)
    ));
    ...
    for (int32_t i = static_cast<int32_t>(config_.total_gpu_blocks) - 1; i >= 0; --i) {
...
PagedKVCache::~PagedKVCache() {
    ...
    if (h_pinned_block_tables_) cudaFreeHost(h_pinned_block_tables_);
    if (h_pinned_context_lens_) cudaFreeHost(h_pinned_context_lens_);
    if (d_block_tables_) cudaFree(d_block_tables_);
    if (d_context_lens_) cudaFree(d_context_lens_);

    if (d_key_pool_) allocator_.free(d_key_pool_);
    if (d_val_pool_) allocator_.free(d_val_pool_);
}

uint64_t PagedKVCache::compute_block_hash(std::span<const int32_t> tokens, uint64_t parent_hash) noexcept {
    uint64_t hash = parent_hash ^ 0x9e3779b97f4a7c15ULL;
    for (int32_t token : tokens) {
        hash ^= std::hash<int32_t>{}(token) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
    }
    return hash;
}
...
int32_t PagedKVCache::allocate_physical_block() {
    if (free_block_stack_.empty()) {
        if (!evict_lru_block()) {
            throw OutOfPagesException(fmt::format(
                "OutOfPagesException: Physical KV Cache pool exhausted! All {} blocks in active use.",
...
void PagedKVCache::free_physical_block(int32_t block_id) {
    if (block_id < 0 || static_cast<size_t>(block_id) >= config_.total_gpu_blocks) return;
...
uint32_t PagedKVCache::register_sequence_with_prefix(uint64_t sequence_id, std::span<const int32_t> prompt_tokens) {
    std::lock_guard<std::mutex> lock(mutex_);

    if (page_tables_.find(sequence_id) != page_tables_.end()) return 0;
    ...
    if (config_.enable_prefix_caching) {
        for (uint32_t b = 0; b < full_blocks; ++b) {
            auto block_tokens = prompt_tokens.subspan(b * config_.block_size, config_.block_size);
    ...
    for (uint32_t b = 0; b < un_matched_full_blocks; ++b) {
        ...
        auto block_tokens = prompt_tokens.subspan(block_idx * config_.block_size, config_.block_size);
        uint64_t block_hash = compute_block_hash(block_tokens, current_parent_hash);
...
void PagedKVCache::append_tokens(uint64_t sequence_id, uint32_t num_tokens) {
    ...
    if (it == page_tables_.end()) {
        throw std::runtime_error(fmt::format("Sequence ID {} not registered!", sequence_id));
    }
    ...
    if (num_tokens <= available) {
        ...
        uint32_t additional_blocks = (remaining + config_.block_size - 1) / config_.block_size;
...
void PagedKVCache::fork_sequence(uint64_t parent_sequence_id, uint64_t child_sequence_id) {
    std::lock_guard<std::mutex> lock(mutex_);

    auto parent_it = page_tables_.find(parent_sequence_id);
    if (parent_it == page_tables_.end()) {
        throw std::runtime_error(fmt::format("Parent Sequence ID {} not found!", parent_sequence_id));
    }
...
KVCacheBatchDescriptor PagedKVCache::prepare_batch_metadata(
    ...
    if (batch_size > config_.max_supported_batch_size) {
        throw std::out_of_range("Requested batch size exceeds max_supported_batch_size");
    }
    ...
    for (uint64_t seq_id : active_sequence_ids) {
        ...
        if (it == page_tables_.end()) {
            throw std::runtime_error(fmt::format("prepare_batch_metadata: Sequence ID {} not found!", seq_id));
        }
        max_blocks_in_batch = std::max(max_blocks_in_batch, it->second.size());
    }

    if (max_blocks_in_batch > config_.max_blocks_per_sequence) {
        throw std::out_of_range("Sequence blocks count exceeds max_blocks_per_sequence limit");
    }

    // 1. Flatten host-side block tables and compute context lengths in pinned memory
    std::memset(h_pinned_block_tables_, -1, batch_size * max_blocks_in_batch * sizeof(int32_t));
    ...
    for (size_t i = 0; i < batch_size; ++i) {
        ...
        uint32_t full_tokens = static_cast<uint32_t>(blocks.size() - 1) * config_.block_size;
        uint32_t last_block_tokens = physical_blocks_[blocks.back()].num_tokens_filled;
        h_pinned_context_lens_[i] = static_cast<int32_t>(full_tokens + last_block_tokens);
    }

    // 2. Asynchronously copy metadata from Pinned Host Memory -> Device Staging Buffers
    const size_t bytes_block_table = batch_size * max_blocks_in_batch * sizeof(int32_t);
    const size_t bytes_context_lens = batch_size * sizeof(int32_t);

    CUDA_CHECK_METADATA(cudaMemcpyAsync(
        d_block_tables_, 
        h_pinned_block_tables_, 
        bytes_block_table, 
...
const std::vector<int32_t>& PagedKVCache::get_block_table(uint64_t sequence_id) const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = page_tables_.find(sequence_id);
    if (it == page_tables_.end()) {
        throw std::runtime_error(fmt::format("Sequence ID {} not found!", sequence_id));
    }
    return it->second;
}

void* PagedKVCache::get_key_block_ptr(int32_t block_id, uint32_t layer_idx) const {
    size_t block_offset = (static_cast<size_t>(block_id) * bytes_per_block_) + (layer_idx * layer_block_bytes_);
    return static_cast<uint8_t*>(d_key_pool_) + block_offset;
}

void* PagedKVCache::get_value_block_ptr(int32_t block_id, uint32_t layer_idx) const {
    size_t block_offset = (static_cast<size_t>(block_id) * bytes_per_block_) + (layer_idx * layer_block_bytes_);
...
KVCacheStats PagedKVCache::get_stats() const {
    ...
    stats.allocated_blocks = stats.total_blocks - stats.free_blocks - stats.cached_prefix_blocks;
    stats.total_memory_bytes = stats.total_blocks * bytes_per_block_ * 2;
    stats.used_memory_bytes = (stats.allocated_blocks + stats.cached_prefix_blocks) * bytes_per_block_ * 2;
    stats.pool_utilization_ratio = static_cast<double>(stats.total_blocks - stats.free_blocks) / static_cast<double>(stats.total_blocks);
