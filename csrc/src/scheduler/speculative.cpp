...
int32_t
SpeculativeEngine::sample_from_distribution(const std::vector<float> &probs) {
...
int32_t SpeculativeEngine::sample_adjusted_distribution(
    const std::vector<float> &target_p, const std::vector<float> &draft_q) {
...
SpeculativeVerificationResult SpeculativeEngine::verify_candidates(
    ...
    const std::vector<std::vector<float>> &target_probs, int32_t eos_token_id) {
  ...
  if (draft_probs.size() != K || target_probs.size() < K + 1) {
    throw std::invalid_argument("Incompatible draft and target tensor "
                                "dimensions in SpeculativeEngine.");
  }
  ...
  for (size_t i = 0; i < K; ++i) {
    ...
    if (r <= acceptance_ratio) {
      ...
      // Token Rejected at index i: Sample replacement from p'(x) = max(0, p(x)
      // - q(x))
      result.final_token =
          sample_adjusted_distribution(target_probs[i], draft_probs[i]);
...
SpeculativeVerificationResult SpeculativeEngine::verify_candidates_greedy(
  ...
  for (size_t i = 0; i < K; ++i) {
    ...
    auto max_it =
        std::max_element(target_probs[i].begin(), target_probs[i].end());
    int32_t target_argmax =
        static_cast<int32_t>(std::distance(target_probs[i].begin(), max_it));
  ...
  auto bonus_it =
      std::max_element(target_probs[K].begin(), target_probs[K].end());
  result.final_token =
      static_cast<int32_t>(std::distance(target_probs[K].begin(), bonus_it));
  if (result.final_token == eos_token_id) {
    result.end_of_sequence = true;
  }
