...
ContinuousBatcher::ContinuousBatcher(size_t max_num_seqs, size_t max_num_batched_tokens)
    : max_num_seqs_(max_num_seqs), max_num_batched_tokens_(max_num_batched_tokens) {}
...
bool ContinuousBatcher::cancel_request(uint64_t request_id) {
    ...
    running_requests_.erase(
        std::remove_if(running_requests_.begin(), running_requests_.end(),
                       [request_id](const std::shared_ptr<Request>& r) { return r->id == request_id; }),
...
void ContinuousBatcher::preempt_requests(size_t required_tokens, BatchStepResult& result) {
    // Sort running requests by lowest priority first for preemption
    std::sort(running_requests_.begin(), running_requests_.end(),
...
void ContinuousBatcher::update_requests_state(const std::vector<uint64_t>& finished_ids) {
    std::lock_guard<std::mutex> lock(mutex_);
