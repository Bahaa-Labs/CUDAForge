...
/*
    Helper utility to convert numerical data type code into a human-readable
   string.
 */
static std::string dtype_code_to_string(int32_t dtype_code) {
  ...
  case 3:
    return "INT8";
  default:
    return "UNKNOWN(" + std::to_string(dtype_code) + ")";
  }
}

/*
    Dump active autotuner cache statistics to standard output for debugging and
   profiling.
 */
void dump_autotuner_cache(KernelDispatcher &dispatcher) {
  ...
  std::cout << "---------------------------------------------------------------"
               "---------\n";
  std::cout << std::left << std::setw(20) << "Operation" << std::setw(8) << "M"
            << std::setw(8) << "N" << std::setw(8) << "K" << std::setw(12)
            << "Precision"
            << "Elected Variant\n";
  std::cout << "---------------------------------------------------------------"
               "---------\n";
