...
void ModelRunner::filter_cancelled_requests(scheduler::BatchStepResult& batch_result) {
    std::lock_guard<std::mutex> lock(cancel_mutex_);
    if (cancelled_requests_.empty()) return;

    auto is_cancelled_fn = [this](const std::shared_ptr<scheduler::Request>& req) {
...
size_t ModelRunner::step() {
    ...
    size_t total_active = step_result.prefill_requests.size() + step_result.decode_requests.size();
    if (total_active == 0) {
        return 0;
    }
    ...
    for (auto& req : step_result.prefill_requests) {
        int32_t sampled_token = req->prompt_tokens.empty() ? 0 : (req->prompt_tokens.back() + 1) % vocab_size_;
        req->generated_tokens.push_back(sampled_token);

        bool is_final = (sampled_token == eos_token_id_) || 
                        (req->generated_tokens.size() >= static_cast<size_t>(req->max_new_tokens));
    ...
    for (auto& req : step_result.decode_requests) {
        int32_t last_token = req->generated_tokens.empty() ? 0 : req->generated_tokens.back();
