...
class GenerationRequest(BaseModel):
    ...
    def validate_prompt_not_empty(cls, value: str) -> str:
        if not value:
            raise ValueError("Prompt text cannot consist solely of whitespace or be empty.")
        return value
...
class TokenStreamChunk(BaseModel):
    ...
    """
    model_config = ConfigDict(frozen=True)  # Immutability for safety during async yields
    ...
    logprob: Optional[float] = Field(default=None, description="Log probability of the token.")
    step_latency_ms: float = Field(..., description="Wall-clock time taken to generate this specific token.")
    is_final: bool = Field(default=False, description="Flag indicating if this is the final token in the sequence.")
...
class GenerationResponse(BaseModel):
    ...
    request_id: str = Field(..., description="Unique trace ID for the inference request.")
    prompt: str = Field(..., description="The original input prompt.")
    generated_text: str = Field(..., description="The complete, concatenated output text.")
    finish_reason: str = Field(..., description="Reason for generation halt (e.g., 'stop', 'length').")
    tokens_generated: int = Field(..., description="Total number of tokens yielded.")
    total_latency_ms: float = Field(..., description="End-to-end request processing time.")
    time_to_first_token_ms: float = Field(..., description="Latency until the first token was generated (TTFT).")