...
from eval_platform.statistics.analyzer import LatencyDistributionSummary, StatisticalAnalyzer
...
from eval_platform.workloads.dataset_loader import EvaluationSample, WorkloadDatasetLoader
...
class EvaluationOrchestrator:
    ...
    def __init__(
        ...
        self.logger = ExperimentLogger(experiment_name=experiment_name, store_root=store_root)
    ...
    def run_end_to_end_benchmark(
        ...
        with self.logger.start_run(run_name="eval_suite", seed=seed) as run:
            ...
            self.logger.log_metrics({
                ...
                "prefill_tps": last_profile_res.prefill_tps if last_profile_res else 0.0,
                "generation_tps": last_profile_res.generation_tps if last_profile_res else 0.0,
            })
...
class BenchmarkOrchestrator:
    ...
    def estimate_vram_bytes(self, config: BenchmarkPointConfig) -> int:
        ...
        bytes_per_elem = 2 if config.precision in (PrecisionMode.FP16, PrecisionMode.INT8) else 4
        return config.batch_size * (config.prompt_len + config.gen_len) * 1024 * bytes_per_elem
